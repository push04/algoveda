...
CREATE POLICY "Users see own watchlist items" ON public.watchlist_items
  FOR ALL USING (
    auth.uid() IN (SELECT user_id FROM public.watchlists WHERE id = watchlist_id)
  );

-- Portfolios policies
CREATE POLICY "Users see own portfolios" ON public.portfolios
  FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users see own holdings" ON public.portfolio_holdings
  FOR ALL USING (
...
CREATE POLICY "Public strategies are viewable by all" ON public.strategies
  FOR SELECT USING (is_public = true);

-- Backtest policies
CREATE POLICY "Users see own backtest jobs" ON public.backtest_jobs
  FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Users see own backtest results" ON public.backtest_results
  FOR ALL USING (auth.uid() = user_id);

-- Paper trading policies
CREATE POLICY "Users see own paper orders" ON public.paper_orders
  FOR ALL USING (auth.uid() = user_id);

-- Screener policies
CREATE POLICY "Users manage own saved screens" ON public.saved_screens
  FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users manage own email prefs" ON public.email_preferences
  FOR ALL USING (auth.uid() = id);
...
CREATE POLICY "Public reports are viewable by all" ON public.research_reports
  FOR SELECT USING (is_public = true);
...
VALUES 
  ('Explorer', 'explorer', 0, 0, 
   '{"backtests": 5, "screener": "basic", "data": "delayed", "digest": "daily"}',
   5, 1, 1440),
  ('Researcher', 'researcher', 49900, 499000,
   '{"backtests": 20, "screener": "full", "data": "realtime", "digest": "4h", "ai_rationale": true, "paper_trading": true}',
   20, 5, 240),
  ('Pro Analyst', 'pro-analyst', 149900, 1499000,
   '{"backtests": -1, "screener": "full", "data": "realtime", "digest": "1h", "ai_rationale": true, "paper_trading": true, "portfolio_analytics": true, "custom_strategies": true, "options_analysis": true, "sector_signals": true}',
   -1, 10, 60),
  ('Institution', 'institution', 499900, 4999000,
   '{"backtests": -1, "screener": "full", "data": "realtime", "digest": "1h", "api_access": true, "team_seats": 5, "priority_support": true, "custom_branding": true}',
   -1, 50, 15);
...
BEGIN
  ...
  VALUES (NEW.id, 'Paper Trading', 'paper');
